...
const { download, parseMention, getBuffer } = require("./function");
const fs = require("fs");
const { statistics } = require("../core/database");
let config = require("../core/config");
const generateUrlInfo = async (
  jid,
  text,
  title = "enak mas",
  description = "nganu mas",
  jpegThumbnail = "./src/logo.jpg",
  quoted = {}
) => {
  const ms = generateWAMessageFromContent(
    jid,
    {
      extendedTextMessage: {
        text,
...
async function sendFile(jid, path, quoted, options = {}) {
  ...
  if (options.audio)
    opt["audio"] = Buffer.isBuffer(path)
      ? { buffer: path, mimetype }
      : { url: path, mimetype };
  if (options.document)
    opt["document"] = Buffer.isBuffer(path)
      ? { buffer: path, mimetype: options.mimetype }
      : { url: path, mimetype: options.mimetype };
  if (options.image)
    opt["image"] = Buffer.isBuffer(path)
      ? { buffer: path, mimetype: options.mimetype }
      : { url: path, mimetype: options.mimetype };
  await client.sendMessage(jid, opt, { quoted }).then(() => {
    try {
  ...
  let msg = WAProto.Message.fromObject({
    groupInviteMessage: WAProto.GroupInviteMessage.fromObject({
      ...
      jpegThumbnail: jpegThumbnail
        ? (await getBuffer(jpegThumbnail)).buffer
        : "",
      caption,
    }),
  });
...
async function sendListM(jid, button, rows, quoted, options = {}) {
  ...
  let waMessageList = generateWAMessageFromContent(jid, messageList, {
    quoted,
    userJid: jid,
    contextInfo: { ...options },
  });
  return await client.relayMessage(jid, waMessageList.message, {
    messageId: waMessageList.key.id,
  });
}
...
async function sendFileFromUrl(
  ...
  try {
    ...
    if (mimetype == "image/gif" || options.gif) {
      ...
      await client.relayMessage(jid, media.message, {
      ...
      const message = await prepareWAMessageMedia(
        {
          [options.audio ? "audio" : "document"]: { url: filepath },
          mimetype: mimetype,
      ...
      let media = generateWAMessageFromContent(jid, msg, {
        quoted,
    ...
    client.sendMessage(
      jid,
      { text: `error nganu => ${util.format(error)} ` },
      { quoted }
    );
  }
}
